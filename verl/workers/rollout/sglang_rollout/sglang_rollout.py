...
class SGLangRollout(BaseRollout):
    ...
    async def _server_by_close_chat(self, messages, model='gpt-4o-2024-11-20', app_key=None, url=None):
        ...
        while current_turns < self.config.multi_turn.max_turns: 
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                try:
                    ...
                    user_response = await self._server_by_close_chat(user_messages)
                    print(f'id: {_req.request_id} User response:{repr(user_response)}')
                except Exception as e:
                    logger.warning(f"req_id {_req.request_id} failed to get user response or response is empty.")
                    finish_reason_type = FinishReasonTypeEnum.STOP
                    break

                if user_response and user_response['choices'] and len(user_response['choices'][0]['message']['content']) != 0:
                    current_turns += 1
