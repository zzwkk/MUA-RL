...
class AsyncRolloutRequest(BaseModel):
    ...
    def initialize_request(cls, values):
        ...
        tools = [tool.model_dump(exclude_none=True) for tool in tool_schemas] if (tool_schemas := values.get("tool_schemas", [])) else None
        ...
        if not values.get("input_ids") or not values.get("attention_mask"):
            tokenization_dict_with_prompt = tokenizer.apply_chat_template(messages, tools=[tool.model_dump(exclude_none=True) for tool in tool_schemas], add_generation_prompt=True, tokenize=True, return_dict=True, enable_thinking=enable_thinking)
        ...
        values["base_conv_with_gen_prompt_end_pos"] = len(tokenizer.apply_chat_template(BASE_CHAT_HISTORY, tools=tools, add_generation_prompt=True, tokenize=False, enable_thinking=enable_thinking))
        return values
    ...
    def add_assistant_message(
        ...
        content = tokenizer.apply_chat_template([*BASE_CHAT_HISTORY, self.messages[-1]], tools=([tool.model_dump(exclude_none=True) for tool in self.tool_schemas] if self.tool_schemas else None), add_generation_prompt=False, tokenize=False)
    ...
    def add_tool_response_messages(self, tokenizer: PreTrainedTokenizer, contents: list[str]) -> None:
        ...
        content = tokenizer.apply_chat_template([*BASE_CHAT_HISTORY, *self.messages[-len(contents) :]], tools=([tool.model_dump(exclude_none=True) for tool in self.tool_schemas] if self.tool_schemas else None), add_generation_prompt=False, tokenize=False)
    ...
    def finalize(
        ...
        if self.enable_tokenization_sanity_check:
            full_tokens = tokenizer.apply_chat_template([msg.model_dump() for msg in self.messages], tools=([tool.model_dump(exclude_none=True) for tool in self.tool_schemas] if self.tool_schemas else None), add_generation_prompt=False, tokenize=True)
